...
def convert_number(value: Union[int, float], source_base: int, target_base: int) -> Union[int, float]:
    """
    Converts a number from source_base to target_base.

    Args:
        value (Union[int, float]): The number to convert.
    ...
    if isinstance(value, int):
        return _convert_int_to_base(value, source_base, target_base)
    elif isinstance(value, float):
        integer_part, fractional_part = str(value).split('.')
        converted_int = _convert_int_to_base(int(integer_part), source_base, target_base)
        converted_frac = _convert_fractional_to_base(float('0.' + fractional_part), target_base)
        return float(f"{converted_int}.{converted_frac}")
    else:
        raise ValueError("Value must be an integer or float.")

def _convert_int_to_base(value: int, source_base: int, target_base: int) -> str:
    """
    Converts an integer from source_base to target_base.

    Args:
        value (int): The integer to convert.
        source_base (int): The base of the input integer.
        target_base (int): The base to convert to.

    Returns:
        str: The converted integer as a string.
    """
    # Convert to decimal first
    decimal_value = int(str(value), source_base)

    # Now convert from decimal to target_base
    if decimal_value == 0:
        return '0'
    digits = []
    while decimal_value:
        digits.append(str(decimal_value % target_base))
        decimal_value = decimal_value // target_base
    return ''.join(digits[::-1])

def _convert_fractional_to_base(value: float, target_base: int, precision: int = 3) -> str:
    """
    Converts the fractional part of a number to a specified base.

    Args:
        value (float): The fractional part (0 <= value < 1).
        target_base (int): The base to convert to.
        precision (int, optional): Number of fractional digits. Defaults to 3.

    Returns:
        str: The converted fractional part as a string.
    """
    fractional = value
    digits = []
